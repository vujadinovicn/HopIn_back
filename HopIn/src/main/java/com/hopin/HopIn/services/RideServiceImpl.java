		...
		if (dto.getScheduledTime() == null) {
			...
			dto.getPassengers().forEach((UserInRideDTO passenger) -> {
				...
				for (Ride ride : passengersRides) {
					if (ride.getStartTime().plusMinutes(ride.getEstimatedTimeInMinutes())
							.isAfter(ride.getScheduledTime()) && ride.getStartTime().isBefore(ride.getScheduledTime()))
						throw new PassengerAlreadyInRideException();
				}
			});
